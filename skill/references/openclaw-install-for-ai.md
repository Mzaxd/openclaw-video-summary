...
确保当前目录是项目根目录（包含 `skill/` 目录）。

推荐（自动处理 Apple Silicon 的 `mlx-whisper`）：
...
python3 -m pip install "youtube-transcript-api>=0.6.0"
```
...
运行时可通过 `--template-type` 强制指定模板（如 `tutorial`）。
...
原因：脚本设计上不再内置 LLM，总结由 OpenClaw 会话内模型完成。

修复：读取任务目录中的 `summary_task_prompt.md`，在 OpenClaw 会话中继续完成最终总结。
...
```text
你是安装执行代理。严格按 skill/references/openclaw-install-for-ai.md 执行：
